...
import { Search, X, ExternalLink, Code2, Smartphone, Gamepad2, Globe, Terminal, Palette } from "lucide-react";
...
export default function TabsContent() {
  const { repos, starred, issues, orgs, user, reposLoading, starredLoading, issuesLoading, orgsLoading, refreshOrgs, refreshStarred } = useGitHub();
  const { t } = useLang();
  ...
  const tabs = [
    ...
    { key: "starred" as const, label: t.nav.starred, count: starred.length },
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === "repos" && (
            <motion.div key="repos" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
              <RepoGrid projects={repos} loading={reposLoading} searchPlaceholder={t.projects.search} emptyText={t.projects.none} />
            </motion.div>
          )}
          ...
          {activeTab === "starred" && (
            <motion.div key="starred" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
              <RepoGrid
                ...
                searchPlaceholder={t.lang === "it" ? "Cerca starred..." : "Search starred..."}
                emptyText={t.lang === "it" ? "Nessuna repository stellata trovata." : "No starred repositories found."}
              />
            </motion.div>
          )}
